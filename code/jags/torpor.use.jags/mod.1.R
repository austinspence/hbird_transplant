model{
  for(i in 1:torpor.obs){
    ...
    mu[i] <- b0 + b.elev*elev[i] + b.oxygen*oxygen[i] + 
      b.sex*sex[i] + b.mass*mass[i] + b.oxygen.elev*oxygen[i]*elev[i] + 
      b.min.temp*min.temp[i] + b.temp.elev*min.temp[i]*elev[i] + ind[id[i]]
  } 
...
#Priors - bigger due to Andrew's suggestion
  ...
  b.sex ~ dnorm(0, 0.25)
  b.mass ~ dnorm(0, 0.25)
  b.oxygen.elev ~ dnorm(0, 0.25)
  b.min.temp ~ dnorm(0, 0.25)
  b.temp.elev ~ dnorm(0, 0.25)
